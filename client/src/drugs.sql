-- SQL script to remove duplicate entries in the Drugs table based on the Name column
-- This script identifies duplicates by using a Common Table Expression (CTE) and deletes them
-- It assumes that the Drugs table has a primary key column named DrugID
WITH Duplicates AS (
  SELECT
    DrugID,
    ROW_NUMBER() OVER (
      PARTITION BY Name
      ORDER BY DrugID
    ) AS rn
  FROM Drugs
)
DELETE FROM Duplicates WHERE rn > 1;

-- SQL script to create Drugs and DrugInteractions tables, insert initial data, and query interactions
-- Insert initial data into Drugs and DrugInteractions tables
-- Insert Drugs
INSERT INTO Drugs (Name, GenericName, DrugClass) VALUES
('Warfarin', 'Warfarin Sodium', 'Anticoagulant'),
('Aspirin', 'Acetylsalicylic Acid', 'NSAID'),
('Cipro', 'Ciprofloxacin', 'Antibiotic'),
('Lipitor', 'Atorvastatin', 'Statin'),
('Crestor', 'Rosuvastatin', 'Statin'),
('Celexa', 'Citalopram', 'Antidepressant'),
('Metformin', 'Metformin', 'Antidiabetic'),
('Lisinopril', 'Lisinopril', 'ACE Inhibitor'),
('Prilosec', 'Omeprazole', 'Proton Pump Inhibitor'),
('Levothyroxine', 'Levothyroxine Sodium', 'Thyroid Hormone'),
('Metoprolol', 'Metoprolol Tartrate', 'Beta Blocker'),
('Amlodipine', 'Amlodipine Besylate', 'Calcium Channel Blocker'),
('Zocor', 'Simvastatin', 'Statin'),
('Plavix', 'Clopidogrel Bisulfate', 'Antiplatelet'),
('Gabapentin', 'Gabapentin', 'Anticonvulsant'),
('Hydrochlorothiazide', 'Hydrochlorothiazide', 'Diuretic'),
('Lasix', 'Furosemide', 'Loop Diuretic'),
('Albuterol', 'Albuterol Sulfate', 'Bronchodilator'),
('Prednisone', 'Prednisone', 'Corticosteroid'),
('Zoloft', 'Sertraline Hydrochloride', 'Antidepressant'),
('Prozac', 'Fluoxetine Hydrochloride', 'Antidepressant'),
('Cymbalta', 'Duloxetine Hydrochloride', 'Antidepressant'),
('Singulair', 'Montelukast Sodium', 'Leukotriene Receptor Antagonist'),
('Flomax', 'Tamsulosin Hydrochloride', 'Alpha Blocker'),
('Ceftriaxone', 'Ceftriaxone Sodium', 'Cephalosporin Antibiotic'),
('Zithromax', 'Azithromycin Dihydrate', 'Macrolide Antibiotic'),
('Metronidazole', 'Metronidazole', 'Antibiotic'),
('Insulin Glargine', 'Insulin Glargine', 'Long-Acting Insulin'),
('Insulin Lispro', 'Insulin Lispro', 'Rapid-Acting Insulin'),
('Klonopin', 'Clonazepam', 'Benzodiazepine'),
('Ativan', 'Lorazepam', 'Benzodiazepine'),
('Valium', 'Diazepam', 'Benzodiazepine'),
('Xanax', 'Alprazolam', 'Benzodiazepine'),
('Oxycodone', 'Oxycodone Hydrochloride', 'Opioid Analgesic'),
('Norco', 'Hydrocodone-Acetaminophen', 'Opioid Analgesic'),
('Tramadol', 'Tramadol Hydrochloride', 'Opioid Analgesic'),
('Lyrica', 'Pregabalin', 'Anticonvulsant'),
('Ambien', 'Zolpidem Tartrate', 'Sedative-Hypnotic'),
('Wellbutrin', 'Bupropion Hydrochloride', 'Antidepressant'),
('Remeron', 'Mirtazapine', 'Antidepressant'),
('Seroquel', 'Quetiapine Fumarate', 'Antipsychotic'),
('Risperdal', 'Risperidone', 'Antipsychotic'),
('Abilify', 'Aripiprazole', 'Antipsychotic'),
('Lithium Carbonate', 'Lithium Carbonate', 'Mood Stabilizer'),
('Depakote', 'Divalproex Sodium', 'Anticonvulsant/Mood Stabilizer'),
('Adderall', 'Amphetamine-Dextroamphetamine', 'Stimulant'),
('Concerta', 'Methylphenidate', 'Stimulant'),
('Vyvanse', 'Lisdexamfetamine Dimesylate', 'Stimulant'),
('Zyrtec', 'Cetirizine Hydrochloride', 'Antihistamine'),
('Claritin', 'Loratadine', 'Antihistamine'),
('Allegra', 'Fexofenadine Hydrochloride', 'Antihistamine'),
('Flonase', 'Fluticasone Propionate', 'Corticosteroid Nasal Spray'),
('Nasonex', 'Mometasone Furoate', 'Corticosteroid Nasal Spray'),
('Xarelto', 'Rivaroxaban', 'Anticoagulant'),
('Eliquis', 'Apixaban', 'Anticoagulant'),
('Pradaxa', 'Dabigatran Etexilate', 'Anticoagulant'),
('Trulicity', 'Dulaglutide', 'GLP-1 Receptor Agonist'),
('Januvia', 'Sitagliptin Phosphate', 'DPP-4 Inhibitor'),
('Farxiga', 'Dapagliflozin', 'SGLT2 Inhibitor'),
('Invokana', 'Canagliflozin', 'SGLT2 Inhibitor'),
('Actos', 'Pioglitazone Hydrochloride', 'Thiazolidinedione'),
('Glucophage', 'Metformin Hydrochloride', 'Biguanide'),
('Lexapro', 'Escitalopram Oxalate', 'Antidepressant'),
('Pristiq', 'Desvenlafaxine Succinate', 'Antidepressant'),
('Effexor XR', 'Venlafaxine Hydrochloride', 'Antidepressant'),
('Buspar', 'Buspirone Hydrochloride', 'Anxiolytic'),
('Zanaflex', 'Tizanidine Hydrochloride', 'Muscle Relaxant'),
('Flexeril', 'Cyclobenzaprine Hydrochloride', 'Muscle Relaxant'),
('Robaxin', 'Methocarbamol', 'Muscle Relaxant'),
('Celebrex', 'Celecoxib', 'NSAID'),
('Voltaren', 'Diclofenac Sodium', 'NSAID'),
('Lidocaine Patch', 'Lidocaine', 'Topical Analgesic'),
('Bactrim', 'Trimethoprim-Sulfamethoxazole', 'Antibiotic'),
('Augmentin', 'Amoxicillin-Clavulanate Potassium', 'Antibiotic'),
('Cipro HC Otic', 'Ciprofloxacin-Hydrocortisone', 'Otic Antibiotic'),
('Lotrel', 'Amlodipine-Benazepril', 'Combination Antihypertensive'),
('Combivent', 'Ipratropium-Albuterol', 'Bronchodilator Combination'),
('Symbicort', 'Budesonide-Formoterol', 'Corticosteroid-Bronchodilator Combination'),
('Advair Diskus', 'Fluticasone-Salmeterol', 'Corticosteroid-Bronchodilator Combination'),
('Singulair Chewable', 'Montelukast Sodium Chewable', 'Leukotriene Receptor Antagonist'),
('Zostavax', 'Zoster Vaccine Live', 'Vaccine'),
('FluMist', 'Influenza Vaccine Live', 'Vaccine'),
('Gardasil', 'Human Papillomavirus Vaccine', 'Vaccine'),
('Prevnar 13', 'Pneumococcal Conjugate Vaccine', 'Vaccine'),
('Hepatitis B Vaccine', 'Hepatitis B Vaccine Recombinant', 'Vaccine'),
('Shingrix', 'Zoster Vaccine Recombinant', 'Vaccine'),
('Beyaz', 'Drospirenone-Ethinyl Estradiol', 'Oral Contraceptive'),
('Yasmin', 'Drospirenone-Ethinyl Estradiol', 'Oral Contraceptive'),
('Ortho Tri-Cyclen', 'Norgestimate-Ethinyl Estradiol', 'Oral Contraceptive'),
('NuvaRing', 'Etonogestrel-Ethinyl Estradiol', 'Vaginal Contraceptive Ring'),
('Depo-Provera', 'Medroxyprogesterone Acetate', 'Injectable Contraceptive'),
('Plan B One-Step', 'Levonorgestrel', 'Emergency Contraceptive'),
('Aldactone', 'Spironolactone', 'Potassium-Sparing Diuretic'),
('K-Tab', 'Potassium Chloride', 'Electrolyte Supplement'),
('Zantac', 'Ranitidine Hydrochloride', 'H2 Antagonist'),
('Pepcid', 'Famotidine', 'H2 Antagonist'),
('Prilosec OTC', 'Omeprazole Magnesium', 'Proton Pump Inhibitor'),
('Prevacid', 'Lansoprazole', 'Proton Pump Inhibitor'),
('Nexium', 'Esomeprazole Magnesium', 'Proton Pump Inhibitor'),
('Tums', 'Calcium Carbonate', 'Antacid'),
('Maalox', 'Aluminum Hydroxide-Magnesium Hydroxide-Simethicone', 'Antacid'),
('Imodium AD', 'Loperamide Hydrochloride', 'Antidiarrheal'),
('Pepto-Bismol', 'Bismuth Subsalicylate', 'Antidiarrheal'),
('Miralax', 'Polyethylene Glycol 3350', 'Laxative'),
('Colace', 'Docusate Sodium', 'Stool Softener'),
('Metamucil', 'Psyllium Husk', 'Fiber Supplement'),
('Zantac 75', 'Ranitidine Hydrochloride', 'H2 Antagonist'),
('Pepcid AC', 'Famotidine', 'H2 Antagonist'),
('Tagamet HB', 'Cimetidine', 'H2 Antagonist'),
('Gas-X', 'Simethicone', 'Antiflatulent'),
('Beano', 'Alpha-Galactosidase Enzyme', 'Digestive Aid'),
('Lactaid', 'Lactase Enzyme', 'Digestive Aid'),
('Prevacid 24HR', 'Lansoprazole', 'Proton Pump Inhibitor'),
('Nexium 24HR', 'Esomeprazole Magnesium', 'Proton Pump Inhibitor'),
('Zyrtec-D', 'Cetirizine-Pseudoephedrine', 'Antihistamine/Decongestant Combination'),
('Claritin-D', 'Loratadine-Pseudoephedrine', 'Antihistamine/Decongestant Combination'),
('Allegra-D', 'Fexofenadine-Pseudoephedrine', 'Antihistamine/Decongestant Combination'),
('Robitussin DM', 'Dextromethorphan-Guaifenesin', 'Cough Suppressant/Expectorant'),
('Mucinex DM', 'Dextromethorphan-Guaifenesin', 'Cough Suppressant/Expectorant'),
('NyQuil', 'Acetaminophen-Dextromethorphan-Doxylamine', 'Cold/Flu Combination'),
('DayQuil', 'Acetaminophen-Dextromethorphan-Guaifenesin', 'Cold/Flu Combination'),
('Theraflu', 'Acetaminophen-Dextromethorphan-Phenylephrine', 'Cold/Flu Combination'),
('Sudafed PE', 'Phenylephrine Hydrochloride', 'Decongestant'),
('Sudafed 12 Hour', 'Pseudoephedrine Sulfate', 'Decongestant'),
('Afrin', 'Oxymetazoline Hydrochloride', 'Nasal Decongestant Spray'),
('Zicam', 'Zinc Gluconate', 'Cold Remedy'),
('Neosporin', 'Bacitracin-Neomycin-Polymyxin B', 'Topical Antibiotic'),
('Hydrocortisone Cream', 'Hydrocortisone', 'Topical Corticosteroid'),
('Lotrimin AF', 'Clotrimazole Antifungal Cream', 'Antifungal Cream'),
('Tinactin Powder Spray', 'Tolnaftate Powder Spray', 'Antifungal Powder Spray'),
('Monistat 1-Day Treatment', 'Miconazole Nitrate Vaginal Cream', 'Antifungal Vaginal Cream'),
('Monistat 7-Day Treatment', 'Miconazole Nitrate Vaginal Cream', 'Antifungal Vaginal Cream'),
('Lotrimin AF Athlete''s Foot Powder', 'Clotrimazole Antifungal Powder', 'Antifungal Powder'),
('Tinactin Antifungal Powder', 'Tolnaftate Antifungal Powder', 'Antifungal Powder'),
('Hydrocortisone 1% Cream', 'Hydrocortisone Cream', 'Topical Corticosteroid'),
('Neosporin Plus Pain Relief', 'Bacitracin-Neomycin-Polymyxin B-Pramoxine', 'Topical Antibiotic/Pain Relief'),
('Bactroban Ointment', 'Mupirocin Calcium Ointment', 'Topical Antibiotic'),
('Polysporin Ointment', 'Bacitracin-Polymyxin B Ointment', 'Topical Antibiotic'),
('Aloe Vera Gel', 'Aloe Vera Gel', 'Topical Moisturizer'),
('Fentanyl', 'Fentanyl', 'Opioid Analgesic'),
('Journavx', 'Suzetrigine', 'Sodium Channel Blocker'),
('Nucynta', 'Tapentadol', 'Opioid Analgesic'),
('Zohydro ER', 'Hydrocodone Bitartrate', 'Opioid Analgesic'),
('Suboxone', 'Buprenorphine-Naloxone', 'Opioid Dependence Treatment'),
('Methadone', 'Methadone Hydrochloride', 'Opioid Analgesic'),
('Buprenorphine', 'Buprenorphine', 'Opioid Dependence Treatment'),
('Lomotil', 'Diphenoxylate-Atropine', 'Antidiarrheal'),
('Vistaril', 'Hydroxyzine Pamoate', 'Antihistamine/Anxiolytic'),
('Atarax', 'Hydroxyzine Hydrochloride', 'Antihistamine/Anxiolytic'),
('Phenergan', 'Promethazine Hydrochloride', 'Antihistamine/Antiemetic'),
('Compazine', 'Prochlorperazine', 'Antiemetic/Antipsychotic'),
('Zofran', 'Ondansetron Hydrochloride', 'Antiemetic'),
('Reglan', 'Metoclopramide Hydrochloride', 'Antiemetic/Gastroprokinetic'),
('Dulcolax', 'Bisacodyl', 'Stimulant Laxative'),
('Senokot', 'Senna', 'Stimulant Laxative'),
('Ciloxan', 'Ciprofloxacin Ophthalmic', 'Ophthalmic Antibiotic'),
('Vigamox', 'Moxifloxacin Ophthalmic', 'Ophthalmic Antibiotic'),
('Pred Forte', 'Prednisolone Acetate Ophthalmic', 'Ophthalmic Corticosteroid'),
('Tobrex', 'Tobramycin Ophthalmic', 'Ophthalmic Antibiotic'),
('Xalatan', 'Latanoprost Ophthalmic', 'Glaucoma Treatment'),
('Lumigan', 'Bimatoprost Ophthalmic', 'Glaucoma Treatment'),
('Travatan Z', 'Travoprost Ophthalmic', 'Glaucoma Treatment'),
('Restasis', 'Cyclosporine Ophthalmic Emulsion', 'Dry Eye Treatment'),
('Pataday', 'Olopatadine Hydrochloride Ophthalmic', 'Allergic Conjunctivitis Treatment'),
('Zaditor', 'Ketotifen Fumarate Ophthalmic', 'Allergic Conjunctivitis Treatment'),
('Mounjaro', 'Tirzepatide', 'GLP-1 Receptor Agonist'),
('Ozempic', 'Semaglutide', 'GLP-1 Receptor Agonist'),
('Rybelsus', 'Semaglutide Oral', 'GLP-1 Receptor Agonist'),
('Wegovy', 'Semaglutide Injection', 'GLP-1 Receptor Agonist'),
('Qsymia', 'Phentermine-Topiramate', 'Weight Management Medication'),
('Rytary', 'Methamphetamine Hydrochloride', 'Stimulant'),
('Praluent', 'Alirocumab', 'PCSK9 Inhibitor'),
('Repatha', 'Evolocumab', 'PCSK9 Inhibitor'),
('Nexletol', 'Bempedoic Acid', 'Cholesterol Lowering Agent'),
('Nexlizet', 'Bempedoic Acid-Ezetimibe', 'Cholesterol Lowering Combination'),
('Leqvio', 'Inclisiran', 'PCSK9 Inhibitor'),
('Zimura', 'Avacincaptad Pegol', 'Age-Related Macular Degeneration Treatment'),
('Syfovre', 'Pegcetacoplan', 'Age-Related Macular Degeneration Treatment'),
('Caplyta', 'Lumateperone', 'Antipsychotic'),
('Vabysmo', 'Faricimab', 'Age-Related Macular Degeneration Treatment'),
('Evkeeza', 'Evinacumab', 'LDL Cholesterol Lowering Agent'),
('Leqembi', 'Lecanemab', 'Alzheimer''s Disease Treatment'),
('Otezla', 'Apremilast', 'PDE4 Inhibitor for Psoriasis and Psoriatic Arthritis'),
('Zolmitriptan', 'Zolmitriptan', 'Triptan for Migraine Treatment'),
('Zonasimide', 'Zonisamide', 'Anticonvulsant'),
('Sacubitril-Valsartan', 'Sacubitril-Valsartan', 'Heart Failure Treatment'),
('Triamterene-Hydrochlorothiazide', 'Triamterene-Hydrochlorothiazide', 'Combination Diuretic'),
('Mupirocin', 'Mupirocin', 'Topical Antibiotic'),
('Cefuroxime', 'Cefuroxime', 'Cephalosporin Antibiotic'),
('Finasteride', 'Finasteride', '5-Alpha Reductase Inhibitor'),
('Dutasteride', 'Dutasteride', '5-Alpha Reductase Inhibitor'),
('Tadalafil', 'Tadalafil', 'PDE5 Inhibitor for Erectile Dysfunction'),
('Sildenafil', 'Sildenafil', 'PDE5 Inhibitor for Erectile Dysfunction'),
('Vardenafil', 'Vardenafil', 'PDE5 Inhibitor for Erectile Dysfunction'),
('Tretinoin', 'Tretinoin', 'Retinoid for Acne Treatment'),
('Isotretinoin', 'Isotretinoin', 'Retinoid for Severe Acne Treatment'),
('Adapalene', 'Adapalene', 'Topical Retinoid for Acne Treatment'),
('Benzoyl Peroxide', 'Benzoyl Peroxide', 'Topical Acne Treatment'),
('Clindamycin Topical', 'Clindamycin Phosphate Topical', 'Topical Antibiotic for Acne Treatment'),
('Erythromycin Topical', 'Erythromycin Topical', 'Topical Antibiotic for Acne Treatment'),
('Azelaic Acid', 'Azelaic Acid', 'Topical Treatment for Acne and Rosacea'),
('Salicylic Acid', 'Salicylic Acid', 'Topical Acne Treatment'),
('Clobazam', 'Clobazam', 'Benzodiazepine'), 
('Vimpat', 'Lacosamide', 'Anticonvulsant'),
('Fycompa', 'Perampanel', 'Anticonvulsant'),
('Briviact', 'Brivaracetam', 'Anticonvulsant'),
('Aptiom', 'Eslicarbazepine Acetate', 'Anticonvulsant'),
('Onfi', 'Clobazam', 'Benzodiazepine for Seizure Treatment'),
('Clobetasol Propionate', 'Clobetasol Propionate', 'Topical Corticosteroid for Skin Conditions'),
('Halobetasol Propionate', 'Halobetasol Propionate', 'Topical Corticosteroid for Skin Conditions'),
('Tacrolimus Ointment', 'Tacrolimus', 'Topical Immunomodulator for Skin Conditions'),
('Pimecrolimus Cream', 'Pimecrolimus', 'Topical Immunomodulator for Skin Conditions'),
('Calcipotriene Cream', 'Calcipotriene', 'Topical Vitamin D Analog for Psoriasis Treatment'),
('Entresto', 'Sacubitril-Valsartan', 'Heart Failure Treatment'),
('Zarxio', 'Filgrastim-Sndz', 'Biosimilar for Neutropenia Treatment'),
('Truxima', 'Rituximab-Abbs', 'Biosimilar for Cancer and Autoimmune Diseases Treatment'),
('Ogivri', 'Trastuzumab-Dkst', 'Biosimilar for Breast Cancer Treatment'),
('Mvasi', 'Bevacizumab-Awwb', 'Biosimilar for Cancer Treatment'),
('Avsola', 'Infliximab-axxq', 'Biosimilar for Autoimmune Diseases Treatment');

-- Command: sqlcmd -S 'localhost' -U 'SA' -P '1234S@bc' -d 'master' -i /home/abhishek/pharmacytexteditor/client/src/drugs.sql
